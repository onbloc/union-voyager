            ...
            ModuleCall::SubmitTransaction(msgs) => {
                ...
                match batch_submission_result {
                    ...
                    Some(Err(err)) => match err {
                        ...
                        // simulation hits the same VM logic, so it needs the same classification
                        BroadcastTxCommitError::SimulationFailed { log } => {
                            self.handle_gno_tx_failure(log, msgs, "error simulating tx")
                        }
