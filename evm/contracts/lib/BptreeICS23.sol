...
library BptreeIcs23 {
    ...
    function verifyChainedMembership(
        UnionIcs23.ExistenceProof calldata l2Proof,
        ...
        (bytes32 subroot, CalculateRootError rCode) = calculateRoot(l2Proof);
        if (rCode != CalculateRootError.None) {
            return VerifyChainedMembershipError.InvalidProofRoot;
        }

        VerifyExistenceError vCode = verifyNoRootCheck(l2Proof, true, key, value);
        if (vCode != VerifyExistenceError.None) {
            return convertExistenceError(vCode);
        }

        vCode =
            verify(l1Proof, false, root, l1Key, abi.encodePacked(subroot));
        if (vCode != VerifyExistenceError.None) {
            return convertExistenceError(vCode);
        }
    ...
    function verifyChainedNonMembership(
        ...
        VerifyExistenceError vCode = verifyNoRootCheck(
            l1Proof, false, l1Key, abi.encodePacked(subroot)
        );
        if (vCode != VerifyExistenceError.None) {
    ...
    function applyLeafOp(
        ...
        // leafPrefix passed through as-is: checkAgainstSpec only requires it
        // start with 0, real proofs can have longer prefixes.
        bytes32 data = sha256(
            abi.encodePacked(
                leafPrefix, encodedKey, key, encodedValue, hashedValue
            )
        );
    ...
    function calculateRoot(
        ...
        uint256 pathLength = proof.path.length;
    ...
    function checkAgainstSpec(
        ...
        if (
            checkDepth
                && (pathLength < MIN_DEPTH || pathLength > MAX_DEPTH)
        ) {
            return false;
        }
