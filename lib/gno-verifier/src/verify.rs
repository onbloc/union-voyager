...
mod tests {
    ...
    fn verify_works() {
        let header: Header = serde_json::from_str(
            &fs::read_to_string("testdata/gnoland-testnet13-header-381822-381823.json").unwrap(),
        )
        .unwrap();

        let trusted_header: SignedHeader = serde_json::from_str(
            &fs::read_to_string("testdata/gnoland-testnet13-signed-header-381822.json").unwrap(),
        )
        .unwrap();
